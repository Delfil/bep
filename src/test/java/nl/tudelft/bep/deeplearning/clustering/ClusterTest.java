...
public class ClusterTest {
	...
	public void testIndices() {
		...
		assertEquals(expect1_1, this.layer1_1.listIndices());
		assertEquals(expect1_2, this.layer1_2.listIndices());
		assertEquals(expect_root, this.root.listIndices());
	}

	@Test
	public void testSize() {
		assertEquals(2, this.layer1_1.size());
		assertEquals(2, this.layer1_2.size());
		assertEquals(4, this.root.size());
		assertEquals(1, this.point1.size());
	}

	@Test
	public void testEquals() {
		Cluster root_same = new Cluster(0);
		assertFalse(this.root.equals(root_same));

		root_same.addCluster(this.layer1_1);
		root_same.addCluster(this.layer1_2);
		this.root.calculateMean();
		root_same.calculateMean();
		assertTrue(this.root.equals(root_same));

		int wrongType = 0;
		assertFalse(this.root.equals(wrongType));
