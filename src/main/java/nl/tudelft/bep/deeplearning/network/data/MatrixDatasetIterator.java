...
public class MatrixDatasetIterator extends BaseDatasetIterator {
	 ...
	 * 
	 * @param data
	 *            the {@link GeneExpressionDatabaseI} instance to iterate over
	 * @param start
	 *            The percentage of examples to skip from the left
	 * @param end
	 *            The percentage of examples to skip from the right
	 */
	public MatrixDatasetIterator(final GeneExpressionDatabaseI data, final double start, final double end) {
		super(data.getBatchSize(), 0, new MatrixDataFetcher(data, start, end));
		this.numExamples = this.fetcher.totalExamples();
	}

	protected MatrixDatasetIterator(final GeneExpressionDatabaseI data, final MatrixDataFetcher df) {
