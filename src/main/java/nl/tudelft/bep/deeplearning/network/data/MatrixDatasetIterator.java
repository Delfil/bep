...
public class MatrixDatasetIterator extends BaseDatasetIterator {
	 ...
	 * 
	 * @param data
	 *            the {@link GeneExpressionDatabase} instance to iterate over
	 * @param start
	 *            The percentage of examples to skip from the left
	 * @param end
	 *            The percentage of examples to skip from the right
	 */
	public MatrixDatasetIterator(final GeneExpressionDatabase data, final double start, final double end) {
		super(data.getBatchSize(), 0, new MatrixDataFetcher(data, start, end));
		this.numExamples = this.fetcher.totalExamples();
	}

	protected MatrixDatasetIterator(final GeneExpressionDatabase data, final MatrixDataFetcher df) {
