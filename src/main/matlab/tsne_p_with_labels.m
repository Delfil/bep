function ydata = tsne_p_with_labels(P, labels, print_labels, no_dims)
...
% Edited to draw label numbers at point locations
    ...
    for iter=1:max_iter
        ...
        if ~rem(iter, 10) && ~isempty(labels)
            ...
            if print_labels == true
                text(ydata(:,1)+10, ydata(:,2)-10,cellstr(num2str(labels')));
            end
            axis tight
            axis off
            drawnow
